    ...
    public static void display(double x, double y) {
        ...
        okButton.setOnAction(e->{
            if (cBox.getValue()!=null) {
                ...
                ArrayList<String> UnitInfo = HelloApplication.getParamsToChange( Integer.parseInt(strChoice[0]) );
                System.out.println("Було обрано для зміни об'єкт "+ UnitInfo);
                change_parametrs(UnitInfo);
            }
            window.close();
        });
        ...
        deleteButton.setOnAction(e->{
            if (cBox.getValue()!=null) {
                String[] strChoice = cBox.getValue().toString().split(" ");
                int ID = Integer.parseInt(strChoice[0]);
                HelloApplication.DeleteUnit(ID);
                System.out.println("Було видалено об'єкт " + ID);
            }
            window.close();
        });
